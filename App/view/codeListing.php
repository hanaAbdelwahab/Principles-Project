...
try {
    ...
    // Set default empty arrays to prevent undefined variable errors
    $cars = [];
    $brands = [];
    $locations = [];
    $colors = [];
    $unavailableDates = [];
    $dateRange = ['min_start' => '', 'max_end' => ''];
}
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="row">
          ...
          <div class="mb-3">
           <label class="form-label d-flex justify-content-between align-items-center toggle-header" style="cursor: pointer;">
            ...
            <div class="toggle-body">
             <select class="form-select" name="brand" style="color:rgb(219, 218, 218)">
               <option disabled <?= !isset($_GET['brand']) ? 'selected' : '' ?> style="color:black">Choose...</option>
                 <?php foreach ($brands as $brand): ?>
                <option value="<?= htmlspecialchars($brand) ?>" <?= (isset($_GET['brand']) && $_GET['brand'] === $brand) ? 'selected' : '' ?> style="color:black">
    ...
    <!-- Pickup Location -->
         <div class="mb-3">
           ...
           <div class="toggle-body">
             <select class="form-select" name="location" style="color:rgb(219, 218, 218)">
               <option class="choose-option" disabled <?= !isset($_GET['location']) ? 'selected' : '' ?> style="color:black">Choose...</option>
                 <?php foreach ($locations as $loc): ?>
                 <option value="<?= htmlspecialchars($loc) ?>" <?= (isset($_GET['location']) && $_GET['location'] === $loc) ? 'selected' : '' ?> style="color:black">
         ...
         <div class="mb-3">
           ...
           <div class="toggle-body">
             <div class="d-flex gap-2" >
               <input type="date" class="form-control" name="pickup_date" id="pickupDate" placeholder="YYYY-MM-DD" min="<?= $dateRange['min_start'] ?>" max="<?= $dateRange['max_end'] ?>" value="<?= htmlspecialchars($_GET['pickup_date'] ?? '') ?> " style="color:rgb(219, 218, 218)"/>
               <input type="time" class="form-control" name="pickup_time"  style="color:rgb(219, 218, 218)"/>
             </div>
           </div>
         </div>
    ...
    <!-- Drop-off Date & Time -->
        <div class="mb-3">
         ...
         <div class="toggle-body">
           <div class="d-flex gap-2">
             <input type="date" class="form-control" name="dropoff_date" id="dropoffDate" placeholder="YYYY-MM-DD" min="<?= $dateRange['min_start'] ?>" max="<?= $dateRange['max_end'] ?>" value="<?= htmlspecialchars($_GET['dropoff_date'] ?? '') ?>" style="color:rgb(219, 218, 218)"/>
             <input type="time" class="form-control" name="dropoff_time"  style="color:rgb(219, 218, 218)"/>
           </div>
         </div>
       </div>
...
<div class="mb-3">
  ...
  <div class="toggle-body">
    <select class="form-select" name="color"  style="color:rgb(219, 218, 218)">
  <option disabled <?= !isset($_GET['color']) ? 'selected' : '' ?>  style="color:black">Choose color...</option>
  <?php foreach ($colors as $color): ?>
    <option value="<?= htmlspecialchars($color) ?>" <?= (isset($_GET['color']) && $_GET['color'] === $color) ? 'selected' : '' ?> style="color:black">
...
<div class="mb-3">
  ...
  <input type="number"
         ...
         style="width: 100px; color:rgb(219, 218, 218)" />
</div>
      </div>
    ...
    <?php if (!empty($_GET)): ?>
      <a href="codeListing.php" class="btn btn-danger w-100 mt-2" style="background-color:rgba(207, 1, 1, 0.675);">Reset Filters</a>
    <?php endif; ?>
  </form>
</div>
    </div>
        ...
        <div class="card-wrapper" >
          <div class="card glass" >
            ...
            <div class="card-title" style="color:white;">
             ...
             <div><i class="fas fa-star me-1"></i> <?= htmlspecialchars($car->rate ?? '') ?></div>
             <div><i class="fas fa-user-group me-1"></i> <?= htmlspecialchars($car->renters ?? '') ?></div>
...
<div class="modal fade" id="carModal" tabindex="-1" aria-labelledby="carModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl modal-dialog-centered">
    <div class="modal-content glass p-4 shadow-lg border-0 rounded-4">
     <!-- Close Button -->
     <button type="button" class="btn-close position-absolute top-0 end-0 m-3" data-bs-dismiss="modal" aria-label="Close" style="padding:1rem;"></button>
...
<script>
  ...
  const maxAvailableDate = "<?= $dateRange['max_end'] ?>";
</script>
<script>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  toggleBtn.addEventListener('click', () => {
    ...
    icon.classList.toggle('fa-sun', !lightModeOn);
    icon.classList.toggle('fa-moon', lightModeOn);
  });
});
</script>
<script>
flatpickr("#pickupDate", {
  ...
  altInput: true,
  altFormat: "Y-m-d",
  placeholder: "YYYY-MM-DD", // Doesn't work in Flatpickr, just fallback
  onChange: function(selectedDates, dateStr) {
    const dropoff = document.getElementById("dropoffDate")._flatpickr;
